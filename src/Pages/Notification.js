...
const Notification = () => {
    ...
    useDocumentTitle("Notifications | Kritarth Twitter");
                ...
                <div className="mt-10 w-full">
                    {loading ? (
                        ...
                        notifications.map((notification) => {
                            return <NotificationTemplate key={notification._id} notification={notification} />;
                        })
                    )}
                    {!loading && errMessage && <div className="text-center font-medium mt-4">{errMessage}</div>}
                    {!loading && notifications.length === 0 && (
                        <div className="text-center font-medium mt-12">No Notifications</div>
                    )}
                </div>
            </div>
        </div>
