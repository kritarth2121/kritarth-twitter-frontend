...
const Signup = () => {
    ...
    const isEmptyFields =
        !name.trim().length || !username.trim().length || !email.trim().length || !password.trim().length;

    const isPasswordValid = /^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z]).{8,32}$/.test(password);

    useDocumentTitle("Sign up | Kritarth Twitter");
    ...
    return (
        <div className="h-screen flex justify-center back">
            <div className="text-center mt-14">
                <h1 className="text-5xl font-semibold mb-4">Kritarth Twitter</h1>
                {error !== "" && <p className="text-red-600 font-medium max-w-sm">{error}</p>}
                {errorMessage !== "" && <p className="text-red-600 font-medium max-w-sm">{errorMessage}</p>}
