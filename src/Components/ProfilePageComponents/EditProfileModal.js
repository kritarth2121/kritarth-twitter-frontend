...
const EditProfileModal = ({setShowProfileModal, user}) => {
    ...
    const openWidget = (e) => {
        ...
        window.cloudinary
            .createUploadWidget(
                {
                    cloudName: "kritarth-tweet",
                    uploadPreset: "qxyghvdm",
                    cropping: true,
                },
                (error, result) => {
    ...
    return (
        <ModalWrapper callback={() => setShowProfileModal(false)} ariaLabel="Edit Profile">
            <div className="bg-white rounded-md py-2 px-4 w-96 lg:w-auto" onClick={(e) => e.stopPropagation()}>
                <form onSubmit={submitHandler}>
                    <div className="py-1  flex w-full text-lg items-center border-b">
                        <i role="button" aria-label="Close" aria-describedby="Button to close dialog">
                    ...
                    <div className="py-2 px-2 lg:px-0">
                        <div className="flex px-5 lg:px-0 lg:w-1/2 items-center ml-auto mr-auto space-x-2">
                            <img src={profileUrl} alt={user.name} loading="lazy" className="w-24 h-24 rounded-full" />
                            <button onClick={openWidget} className="button-black whitespace-nowrap">
